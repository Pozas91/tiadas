...
from tests.environments.test_deep_sea_treasure_right_down import TestDeepSeaTreasureRightDown


class TestDeepSeaTreasureRightDownStochastic(TestDeepSeaTreasureRightDown):

    def setUp(self):
        # Set initial_seed to 0 to testing.
        self.environment = DeepSeaTreasureRightDownStochastic(seed=0)

    def test_action_space_length(self):
        # By default action space is 3 (RIGHT_PROB, DOWN_PROB, DOWN)
    ...
    def test__next_state(self):
        ...
        next_state = self.environment.next_state(action=self.environment.actions['RIGHT_PROB'])
        self.assertEqual((state[0] + 1, state[1]), next_state)

        # Go to DOWN (increment y axis)
        next_state = self.environment.next_state(action=self.environment.actions['DOWN_PROB'])
